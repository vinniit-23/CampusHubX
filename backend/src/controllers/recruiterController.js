...
export const getAllRecruiters = asyncHandler(async (req, res) => {
  ...
  const recruiters = await Recruiter.find(query)
    ...
    .sort(req.query.search ? { score: { $meta: 'textScore' } } : { companyName: 1 })
    .skip(page.skip)
    .limit(page.limit);
...
export const getRecruiterById = asyncHandler(async (req, res) => {
  ...
  if (!recruiter) {
    return res.status(404).json(
      formatResponse(false, null, 'Recruiter not found', { code: 'NOT_FOUND' })
    );
  }

  res.status(200).json(
    formatResponse(true, recruiter, 'Recruiter retrieved successfully')
  );
});
...
export const updateOwnProfile = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(
    formatResponse(true, recruiter, 'Profile updated successfully')
  );
});
...
export const getDashboard = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    Application.countDocuments({ opportunityId: { $in: opportunityIds }, status: 'pending' }),
    Application.countDocuments({ opportunityId: { $in: opportunityIds }, status: 'shortlisted' }),
    Application.countDocuments({ opportunityId: { $in: opportunityIds }, status: 'accepted' })
  ]);
